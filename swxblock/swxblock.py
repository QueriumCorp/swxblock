...
class SWXBlock(StudioEditableXBlockMixin, ScorableXBlockMixin, XBlock):
    ...
    q_grade_showme_ded = Float(
        ...
        help="Raw points deducted from 3.0 (Default: 3.0)",
        default=3.0,
        scope=Scope.content,
    )
    q_grade_hints_count = Integer(
        help="Number of Hints before deduction", default=2, scope=Scope.content
    )
    q_grade_hints_ded = Float(
        help="Point deduction for using excessive Hints",
        default=1.0,
        scope=Scope.content,
    )
    q_grade_errors_count = Integer(
        help="Number of Errors before deduction", default=2, scope=Scope.content
    )
    q_grade_errors_ded = Float(
        help="Point deduction for excessive Errors", default=1.0, scope=Scope.content
    )
    q_grade_min_steps_count = Integer(
        help="Minimum valid steps in solution for full credit",
        default=3,
        scope=Scope.content,
    )
    q_grade_min_steps_ded = Float(
        help="Point deduction for fewer than minimum valid steps",
        default=0.25,
        scope=Scope.content,
    )
    ...
    q_option_hint = Boolean(
        help='Display Hint button if "True"', default=True, scope=Scope.content
    )
    ...
    q_max_attempts = Integer(
        help="Max question attempts (-1 = Use Course Default)",
        default=-1,
        scope=Scope.content,
    )
    ...
    display_name = String(
        display_name="Display name", default="StepWise", scope=Scope.content
    )

    q_id = String(help="Question ID", default="", scope=Scope.content)
    q_label = String(help="Question label", default="", scope=Scope.content)
    q_stimulus = String(
        help="Stimulus", default="Solve for \\(a\\).", scope=Scope.content
    )
    q_definition = String(
        help="Definition", default="SolveFor[5a+4=2a-5,a]", scope=Scope.content
    )
    ...
    q_grade_hints_ded = Integer(
        help="Deduction for using extra Hints", default=-1, scope=Scope.content
    )
    ...
    xb_user_email = String(
        help="The user's email addr", default="", scope=Scope.user_state
    )
    grade = Float(help="The student's grade", default=-1, scope=Scope.user_state)
    solution = Dict(
        help="The student's last solution", default={}, scope=Scope.user_state
    )
    question = Dict(
        help="The student's current question", default={}, scope=Scope.user_state
    )
    ...
    count_attempts = Integer(
        help="Counted number of questions attempts", default=0, scope=Scope.user_state
    )
    raw_possible = Float(
        help="Number of possible points", default=3, scope=Scope.user_state
    )
    ...
    my_weight = Integer(
        help="Remember weight course setting vs question setting",
        default=-1,
        scope=Scope.user_state,
    )
    my_max_attempts = Integer(
        help="Remember max_attempts course setting vs question setting",
        default=-1,
        scope=Scope.user_state,
    )
    ...
    my_grade_showme_ded = Integer(
        help="Remember grade_showme_ded course setting vs question setting",
        default=-1,
        scope=Scope.user_state,
    )
    my_grade_hints_count = Integer(
        help="Remember grade_hints_count course setting vs question setting",
        default=-1,
        scope=Scope.user_state,
    )
    my_grade_hints_ded = Integer(
        help="Remember grade_hints_ded course setting vs question setting",
        default=-1,
        scope=Scope.user_state,
    )
    my_grade_errors_count = Integer(
        help="Remember grade_errors_count course setting vs question setting",
        default=-1,
        scope=Scope.user_state,
    )
    my_grade_errors_ded = Integer(
        help="Remember grade_errors_ded course setting vs question setting",
        default=-1,
        scope=Scope.user_state,
    )
    my_grade_min_steps_count = Integer(
        help="Remember grade_min_steps_count course setting vs question setting",
        default=-1,
        scope=Scope.user_state,
    )
    my_grade_min_steps_ded = Integer(
        help="Remember grade_min_steps_ded course setting vs question setting",
        default=-1,
        scope=Scope.user_state,
    )
    ...
    variants_attempted = Integer(
        help="Bitmap of attempted variants", default=0, scope=Scope.user_state
    )
    variants_count = Integer(
        help="Count of available variants", default=0, scope=Scope.user_state
    )
    previous_variant = Integer(
        help="Index (q_index) of the last variant used",
        default=-1,
        scope=Scope.user_state,
    )
    ...
    def student_view(self, context=None):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() entered. context={context}".format(
                    context=context
                )
            )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.runtime={a}".format(a=self.runtime)
            )
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.runtime.course_id={a}".format(
                    a=self.runtime.course_id
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.variants_attempted={v}".format(
                    v=self.variants_attempted
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.previous_variant={v}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() max_attempts={a} q_max_attempts={b}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.q_weight was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_weight = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_weight: {t}".format(t=temp_weight)
            )
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.q_max_attempts was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_max_attempts = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_max_attempts: {t}".format(
                    t=temp_max_attempts
                )
            )
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.option_hint was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_option_hint = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_option_hint: {t}".format(
                    t=temp_option_hint
                )
            )
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.option_showme was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_option_showme = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_option_showme: {t}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.q_grade_showme_ded was not defined in this instance: {e}".format(
                        e=e
                    )
                )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_grade_showme_ded: {t}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.q_grade_hints_count was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_grade_hints_count = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_grade_hints_count: {t}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.q_grade_hints_ded was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_grade_hints_ded = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_grade_hints_ded: {t}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.q_grade_errors_count was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_grade_errors_count = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_grade_errors_count: {t}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.q_grade_errors_ded was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_grade_errors_ded = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_grade_errors_ded: {t}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.q_grade_min_steps_count was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_grade_min_steps_count = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_grade_min_steps_count: {t}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() self.q_grade_min_steps_ded was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_grade_min_steps_ded = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_grade_min_steps_ded: {t}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_weight was not defined in this instance: {e}".format(
                        e=e
                    )
                )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_weight: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_max_attempts was not defined in this instance: {e}".format(
                        e=e
                    )
                )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_max_attempts: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_option_showme was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_course_stepwise_option_showme = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_option_showme: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_option_hint was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_course_stepwise_option_hint = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_option_hint: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_settings_grade_hints_count was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_course_stepwise_grade_hints_count = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_grade_hints_count: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_grade_showme_ded was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_course_stepwise_grade_showme_ded = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_grade_showme_ded: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_grade_hints_ded was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_course_stepwise_grade_hints_ded = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_grade_hints_ded: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_grade_errors_count was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_course_stepwise_grade_errors_count = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_grade_errors_count: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_grade_errors_ded was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_course_stepwise_grade_errors_ded = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_grade_errors_ded: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_grade_min_steps_count was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_course_stepwise_grade_min_steps_count = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_grade_min_steps_count: {s}".format(
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() course.stepwise_grade_min_steps_ded was not defined in this instance: {e}".format(
                        e=e
                    )
                )
            temp_course_stepwise_grade_min_steps_ded = -1
        if DEBUG:
            logger.info(
                "SWXBlock student_view() temp_course_stepwise_grade_min_steps_ded: {s}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.my_weight={m}".format(m=self.my_weight)
            )
        ...
        if temp_max_attempts != -1:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() my_max_attempts={a} temp_max_attempts={m}".format(
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() my_max_attempts={a} temp_course_stepwise_max_attempts={m}".format(
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock student_view() my_max_attempts={a} course.max_attempts={m}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.my_option_hint={m}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.my_grade_showme_ded={m}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.my_grade_hints_count={m}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.my_grade_hints_ded={m}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.my_grade_errors_count={m}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.my_grade_errors_ded={m}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.my_grade_min_steps_count={m}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.my_grade_min_steps_ded={m}".format(
        ...
        if (
            len(self.q_definition) > 0
            and len(self.q1_definition) > 0
            and len(self.q2_definition) > 0
            and len(self.q3_definition) > 0
            and len(self.q4_definition) > 0
            and len(self.q5_definition) > 0
            and len(self.q6_definition) > 0
            and len(self.q7_definition) > 0
            and len(self.q8_definition) > 0
            and len(self.q9_definition) > 0
        ):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() self.variants_count={c}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock student_view() pick_variant selected q_index={i} question={q}".format(
        ...
        try:
            ...
            if response.status_code == 200:
                frag.add_css_url(
                    f"//cdn.{settings.LMS_BASE}/swxblock/static/css/swxstudent.css"
                )
            else:
                logger.warning(
                    f"SWXBlock student_view() failed to load css: {response.status_code}"
                )
        ...
        frag.add_javascript_url(
            "//www.gstatic.com/firebasejs/4.4.0/firebase.js"
        )  # For qEval client-side logging
        frag.add_javascript_url(
            "//stepwiseai.querium.com/client/querium-stepwise-1.6.9.js"
        )  # 1.6.9.1 corrects a bug in hints looping

        frag.add_javascript(self.resource_string("static/js/src/swxstudent.js"))
        if DEBUG:
            logger.info("SWXBlock student_view() calling frag.initialize_js")
    ...
    def publish_grade(self):
        if DEBUG:
            logger.info(
                "SWXBlock publish_grade() self.raw_earned={e} self.weight={w}".format(
        ...
        self.runtime.publish(
            self, "grade", {"value": self.raw_earned, "max_value": self.weight}
        )
    ...
    def get_data(self, msg, suffix=""):
        if DEBUG:
            logger.info("SWXBlock get_data() entered. msg={msg}".format(msg=msg))
        ...
        if DEBUG:
            logger.info("SWXBlock get_data() data={d}".format(d=data))
    ...
    def save_grade(self, data, suffix=""):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() self.max_attempts={a}".format(
                    a=self.max_attempts
                )
            )
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() self.q_weight was not defined: {e}".format(
                        e=e
                    )
                )
            q_weight = 1.0
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() self.q_grade_showme_dev was not defined: {e}".format(
                        e=e
                    )
                )
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() self.q_grade_hints_count was not defined: {e}",
                    format(e=e),
                )
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() self.q_grade_hints_ded was not defined: {e}".format(
                        e=e
                    )
                )
            q_grade_hints_ded = -1
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() self.q_grade_errors_count was not defined: {e}".format(
                        e=e
                    )
                )
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() self.q_grade_errors_ded was not defined: {e}".format(
                        e=e
                    )
                )
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() self.q_grade_min_steps_count was not defined: {e}".format(
                        e=e
                    )
                )
        ...
        try:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() self.q_grade_min_steps_ded was not defined: {e}".format(
                        e=e
                    )
                )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() count valid_steps data={d}".format(d=data)
            )
        step_details = data["stepDetails"]
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() count valid_steps step_details={d}".format(
                    d=step_details
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() count valid_steps len(step_details)={l}".format(
                    l=len(step_details)
                )
            )
        for c in range(len(step_details)):
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() count valid_steps begin examine step c={c} step_details[c]={d}".format(
                        c=c, d=step_details[c]
                    )
                )
            for i in range(len(step_details[c]["info"])):
                if DEBUG:
                    logger.info(
                        "SWXBlock save_grade() count valid_steps examine step c={c} i={i} step_details[c]['info']={s}".format(
                            c=c, i=i, s=step_details[c]["info"]
                        )
                    )
                if DEBUG:
                    logger.info(
                        "SWXBlock save_grade() count valid_steps examine step c={c} i={i} step_details[c]['info'][i]={s}".format(
                            c=c, i=i, s=step_details[c]["info"][i]
                        )
                    )
                ...
                if step_status == 0:  # victory valid_steps += 1
                    ...
                    if DEBUG:
                        logger.info(
                            "SWXBlock save_grade() count valid_steps c={c} i={i} victory step found".format(
                                c=c, i=i
                            )
                        )
                    ...
                    if DEBUG:
                        logger.info(
                            "SWXBlock save_grade() count valid_steps c={c} i={i} valid step found".format(
                                c=c, i=i
                            )
                        )
                    ...
                    if DEBUG:
                        logger.info(
                            "SWXBlock save_grade() count valid_steps c={c} i={i} ignoring step_status={s}".format(
                                c=c, i=i, s=step_status
                            )
                        )
                if DEBUG:
                    logger.info(
                        "SWXBlock save_grade() count valid_steps examine step c={c} i={i} step_status={s} valid_steps={v}".format(
                            c=c, i=i, s=step_status, v=valid_steps
                        )
                    )
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() final valid_steps={v}".format(v=valid_steps)
            )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() initial grade={a} errors={b} errors_count={c} hints={d} hints_count={e} showme={f} min_steps={g} valid_steps={h}".format(
                    a=grade,
                    b=data["errors"],
                    c=q_grade_errors_count,
                    d=data["hints"],
                    e=q_grade_hints_count,
                    f=data["usedShowMe"],
                    g=q_grade_min_steps_count,
                    h=valid_steps,
                )
            )
        if data["errors"] > q_grade_errors_count:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() errors test errors_ded={a} grade={b}".format(
                        a=q_grade_errors_ded, b=grade
                    )
                )
        if data["hints"] > q_grade_hints_count:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() hints test hints_ded={a} grade={b}".format(
                        a=q_grade_hints_ded, b=grade
                    )
                )
        if data["usedShowMe"]:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() showme test showme_ded={a} grade={b}".format(
                        a=q_grade_showme_ded, b=grade
                    )
                )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() check on min_steps deduction grade={g} max_grade={m} q_grade_min_steps_count={c} q_grade_min_steps_ded={d} self.my_q_definition={q}".format(
                    g=grade,
                    m=max_grade,
                    c=q_grade_min_steps_count,
                    d=q_grade_min_steps_ded,
                    q=self.my_q_definition,
                )
            )
        if (
            grade >= max_grade
            and valid_steps < q_grade_min_steps_count
            and self.my_q_definition.count("MatchSpec") == 0
            and self.my_q_definition.count("DomainOf") == 0
        ):
            grade = grade - q_grade_min_steps_ded
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() took min_steps deduction after grade={g}".format(
                        g=grade
                    )
                )
        else:
            if DEBUG:
                logger.info(
                    "SWXBlock save_grade() did not take min_steps deduction after grade={g}".format(
                        g=grade
                    )
                )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() final grade={a} q_weight={b}".format(
                    a=grade, b=q_weight
                )
            )
        ...
        self.runtime.publish(
            self,
            "grade",
            {"value": (grade / 3.0) * q_weight, "max_value": 1.0 * q_weight},
        )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() final self.solution={a}".format(a=self.solution)
            )
        ...
        try:
            if self.q_index != -1:
                ...
                if DEBUG:
                    logger.info(
                        "SWXBlock save_grade() record variants_attempted for variant {a}".format(
                            v=self.q_index
                        )
                    )
                ...
                if DEBUG:
                    logger.info(
                        "SWXBlock save_grade() record previous_variant for variant {a}".format(
                            v=self.previous_variant
                        )
                    )
            else:
                if DEBUG:
                    logger.error(
                        "SWXBlock save_grade record variants_attempted for variant -1"
                    )
        except (NameError, AttributeError) as e:
            if DEBUG:
                logger.warning(
                    "SWXBlock save_grade() self.q_index was not defined: {e}".format(
                        e=e
                    )
                )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() final self.count_attempts={a}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() final self.grade={a}".format(a=self.grade)
            )
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() final self.weight={a}".format(a=self.weight)
            )
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() final self.variants_attempted={v}".format(
                    v=self.variants_attempted
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock save_grade() final self.previous_variant={v}".format(
                    v=self.previous_variant
                )
            )
    ...
    def start_attempt(self, data, suffix=""):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock start_attempt() self.count_attempts={c} max_attempts={m}".format(
                    c=self.count_attempts, m=self.max_attempts
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock start_attempt() self.variants_attempted={v}".format(
                    v=self.variants_attempted
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock start_attempt() self.previous_variant={v}".format(
                    v=self.previous_variant
                )
            )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock start_attempt() passed q_index={q}".format(q=data["q_index"])
            )
        self.count_attempts += 1
        if DEBUG:
            logger.info(
                "SWXBlock start_attempt() updated self.count_attempts={c}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock start_attempt() done return_data={return_data}".format(
    ...
    def retry(self, data, suffix=""):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock retry() self.count_attempts={c} max_attempts={m}".format(
                    c=self.count_attempts, m=self.max_attempts
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock retry() self.variants_attempted={v}".format(
                    v=self.variants_attempted
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock retry() pre-pick_question q_index={i}".format(
        ...
        if DEBUG:
            logger.info(
                "SWXBlock retry() post-pick returning self.question={q} return_data={r}".format(
    ...
    def studio_view(self, context=None):
        ...
        frag = Fragment(html.format(self=self))
        frag.add_css_url("//cdn.web.stepwisemath.ai/swxblock/static/css/swxstudio.css")
    ...
    def author_view(self, context=None):
        ...
        frag = Fragment(html.format(self=self))
        frag.add_css_url("//cdn.web.stepwisemath.ai/swxblock/static/css/swxauthor.css")
        frag.add_javascript_url(
            "//cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.1/MathJax.js?config=TeX-MML-AM_HTMLorMML"
        )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock SWXAuthor author_view v={a}".format(a=self.q_definition)
            )
        if DEBUG:
            logger.info(
                "SWXBlock SWXAuthor author_view v1={a} v2={b} v3={c}".format(
                    a=self.q1_definition, b=self.q2_definition, c=self.q3_definition
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock SWXAuthor author_view v4={a} v5={b} v6={c}".format(
                    a=self.q4_definition, b=self.q5_definition, c=self.q6_definition
                )
            )
        if DEBUG:
            logger.info(
                "SWXBlock SWXAuthor author_view v7={a} v8={b} v9={c}".format(
                    a=self.q7_definition, b=self.q8_definition, c=self.q9_definition
                )
            )
        ...
        if DEBUG:
            logger.info(
                "SWXBlock SWXAuthor author_view variants={a}".format(a=variants)
            )
    ...
    def has_submitted_answer(self):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock has_submitted_answer() {a}".format(a=self.is_answered)
            )
    ...
    def weighted_grade(self):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock weighted_grade() earned {e}".format(e=self.raw_earned)
            )
        if DEBUG:
    ...
    def bit_set_one(self, var, bitnum):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock bit_set_one var={v} bitnum={b}".format(v=var, b=bitnum)
            )
        var = var | (1 << bitnum)
        if DEBUG:
    ...
    def bit_is_set(self, var, bitnum):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock bit_is_set var={v} bitnum={b}".format(v=var, b=bitnum)
            )
        result = var & (1 << bitnum)
        if DEBUG:
            logger.info(
                "SWXBlock bit_is_set result={v} b={b}".format(v=result, b=bool(result))
            )
        return bool(result)
    ...
    def pick_variant(self):
        ...
        if DEBUG:
            logger.info(
                "SWXBlock pick_variant() started replacing prev_index={p}".format(
                    p=prev_index
                )
            )
        ...
        if prev_index == -1:
            try:  # use try block in case attribute wasn't saved in previous student work
                ...
                if DEBUG:
                    logger.info(
                        "SWXBlock pick_variant() using previous_variant for prev_index={p}".format(
                            p=prev_index
                        )
                    )
            except (NameError, AttributeError) as e:
                if DEBUG:
                    logger.info(
                        "SWXBlock pick_variant() self.previous_variant does not exist. Using -1: {e}".format(
                            e=e
                        )
                    )
                prev_index = -1
        ...
        if self.bit_count_ones(self.variants_attempted) >= self.variants_count:
            if DEBUG:
                logger.warn(
                    "SWXBlock pick_variant() seen all variants attempted={a} count={c}, clearing variants_attempted".format(
        ...
        if self.variants_count <= 0:
            if DEBUG:
                logger.warn(
                    "SWXBlock pick_variant() bad variants_count={c}, setting to 1.".format(
                        c=self.variants_count
                    )
                )
            self.variants_count = 1
        ...
        while tries < max_tries:
            ...
            if DEBUG:
                logger.info(
                    "SWXBlock pick_variant() try {t}: q_randint={r}".format(
                        t=tries, r=q_randint
                    )
                )
            ...
            if (
                ...
                if DEBUG:
                    logger.info(
                        "SWXBlock pick_variant() try {t}: with bit_count_ones(variants_attempted)={v} < variants_count={c}-1 we won't use the same variant {q} as prev variant".format(
                            t=tries,
            ...
            if not self.bit_is_set(self.variants_attempted, q_index):
                if DEBUG:
                    logger.info(
                        "SWXBlock pick_variant() try {t}: found unattempted variant {q}".format(
                            t=tries, q=q_index
                        )
                    )
                break
        ...
        if tries >= max_tries:
            if DEBUG:
                logger.error(
                    "pick_variant() could not find an unattempted variant of {i} {l} in {m} tries! clearing self.variants_attempted.".format(
                        i=self.q_id, l=self.q_label, m=max_tries
                    )
                )
            q_index = 0  # Default
            self.variants_attempted = 0
        ...
        if DEBUG:
            logger.info(
                "SWXBlock pick_variant() returned question q_index={i} question={q}".format(
