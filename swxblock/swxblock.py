...
class SWXBlock(StudioEditableXBlockMixin, ScorableXBlockMixin, XBlock):
    ...
    def save_grade(self, data, suffix=''):
        ...
        self.solution = data
        self.grade = grade
        logger.info("SWXBlock save_grade() saved self.grade={a} self.solution={b}".format(a=self.grade,b=self.solution))
        ...
        if q_index != -1:
            ...
            logger.info("SWXBlock save_grade() record variants_attempted for variant {a}".format(a=q_index))
            self.previous_variant = q_index
            logger.info("SWXBlock save_grade() record previous_variant for variant {a}".format(a=self.previous_variant))
        else:
